...
from src.energyml.utils.epc import get_obj_identifier, \
    gen_energyml_object_path, EpcExportVersion
from src.energyml.utils.introspection import epoch_to_date, epoch, gen_uuid, get_obj_pkg_pkgv_type_uuid_version, \
    get_obj_uri
...
def test_get_obj_pkg_pkgv_type_uuid_version_obj_201():
    domain, domain_version, object_type, obj_uuid, obj_version = get_obj_pkg_pkgv_type_uuid_version(fi)
...
def test_get_obj_pkg_pkgv_type_uuid_version_obj_22():
    domain, domain_version, object_type, obj_uuid, obj_version = get_obj_pkg_pkgv_type_uuid_version(tr)
    assert domain == "resqml"
...
def test_get_obj_uri():
    ...
    assert str(get_obj_uri(tr, "/MyDataspace/")) == f"eml:///dataspace('/MyDataspace/')/resqml22.TriangulatedSetRepresentation({tr.uuid})"

    assert str(get_obj_uri(fi)) == f"eml:///resqml20.obj_FaultInterpretation(uuid={fi.uuid},version='{fi.object_version}')"
    assert str(get_obj_uri(fi, "/MyDataspace/")) == f"eml:///dataspace('/MyDataspace/')/resqml20.obj_FaultInterpretation(uuid={fi.uuid},version='{fi.object_version}')"


def test_gen_energyml_object_path():
    assert gen_energyml_object_path(tr) == f"TriangulatedSetRepresentation_{tr.uuid}.xml"
    assert gen_energyml_object_path(tr, EpcExportVersion.EXPANDED) == f"namespace_resqml22/{tr.uuid}/TriangulatedSetRepresentation_{tr.uuid}.xml"
