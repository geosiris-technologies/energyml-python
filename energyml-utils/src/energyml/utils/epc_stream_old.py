...
from energyml.utils.epc_utils import gen_rels_path_from_obj_path
...
class EpcObjectMetadata:
    ...
    Identifier is generated as uuid.version
    """
...
class _MetadataManager:
    ...
    def __init__(self, zip_accessor: _ZipFileAccessor, stats: EpcStreamingStats):
        ...
        self._export_version = EpcExportVersion.CLASSIC  # Store export version
    ...
    def _extract_object_info_fast(
        ...
        try:
            ...
            with zf.open(file_path) as f:
                ...
                obj_type = get_obj_type_from_content_or_qualified_type(content_type)
    ...
    def gen_rels_path_from_metadata(self, metadata: EpcObjectMetadata) -> str:
        ...
        return gen_rels_path_from_obj_path(obj_path=metadata.file_path)
...
class _StructureValidator:
    """
    Internal helper class for validating and repairing EPC file structure.

    Ensures compliance with EPC v1.0 specification by validating:
    - Presence of [Content_Types].xml with correct default types
    - Presence of _rels/.rels with Core Properties relationship
    - Presence of Core Properties file
    - Proper URI encoding and Part Name conventions

    This class provides idempotent repair operations that can be safely
    called multiple times without corrupting the file.
    """

    # EPC specification constants
    ...
    def __init__(
        self, zip_accessor: _ZipFileAccessor, metadata_manager: _MetadataManager, export_version: EpcExportVersion
    ):
        """
    ...
    def validate_and_repair(self, is_new_file: bool = False) -> Dict[str, bool]:
        """
        Validate EPC structure and repair if necessary.

        This method is idempotent - can be called multiple times safely.

        Args:
            is_new_file: If True, bootstrap a complete minimal structure

        Returns:
            Dictionary with validation/repair results:
            {
        ...
        """
        results = {"content_types_ok": False, "root_rels_ok": False, "core_props_ok": False, "repaired": False}

        if is_new_file:
            logging.info("[EPC Structure] Bootstrapping new EPC file with minimal valid structure")
            self._bootstrap_new_file()
            results.update({"content_types_ok": True, "root_rels_ok": True, "core_props_ok": True, "repaired": True})
        else:
            logging.info("[EPC Structure] Validating existing EPC file structure")

            # Check Content Types
            results["content_types_ok"] = self._validate_content_types()
            if not results["content_types_ok"]:
                logging.warning("[EPC Structure] Content Types invalid, repairing...")
                self._repair_content_types()
                results["repaired"] = True

            # Check Root Relationships
            results["root_rels_ok"] = self._validate_root_relationships()
            if not results["root_rels_ok"]:
                logging.warning("[EPC Structure] Root relationships invalid, repairing...")
                self._repair_root_relationships()
                results["repaired"] = True

            # Check Core Properties
            results["core_props_ok"] = self._validate_core_properties()
            if not results["core_props_ok"]:
                logging.warning("[EPC Structure] Core Properties missing or invalid, repairing...")
                self._repair_core_properties()
                results["repaired"] = True

        if results["repaired"]:
            logging.info("[EPC Structure] Structure validation complete - repairs applied")
        else:
            logging.info("[EPC Structure] Structure validation complete - no repairs needed")

        return results

    def _bootstrap_new_file(self) -> None:
        """Bootstrap a new EPC file with complete minimal structure."""
        with zipfile.ZipFile(self.zip_accessor.epc_file_path, "w") as zf:
            # 1. Create Core Properties
            core_props = self._create_default_core_properties()
            core_props_path = gen_core_props_path(self.export_version)
            core_props_xml = serialize_xml(core_props)
            zf.writestr(core_props_path, core_props_xml)
            logging.info(f"[EPC Structure] Created Core Properties at {core_props_path}")

            # 2. Create Content Types with defaults
            content_types = Types(
                default=[
                    Default(extension="rels", content_type=self.RELS_CONTENT_TYPE),
                    Default(extension="xml", content_type="application/xml"),
                ],
                override=[Override(part_name=f"/{core_props_path}", content_type=self.CORE_PROPS_CONTENT_TYPE)],
            )
            content_types_xml = serialize_xml(content_types)
            zf.writestr(get_epc_content_type_path(), content_types_xml)
            logging.info("[EPC Structure] Created [Content_Types].xml with default types")

            # 3. Create Root Relationships
            root_rels = Relationships(
                relationship=[
                    Relationship(id="CoreProperties", type_value=self.CORE_PROPS_REL_TYPE, target=core_props_path)
                ]
            )
            root_rels_xml = serialize_xml(root_rels)
            zf.writestr("_rels/.rels", root_rels_xml)
            logging.info("[EPC Structure] Created _rels/.rels with Core Properties relationship")

        # Update metadata manager
        self.metadata_manager._core_props = core_props
        self.metadata_manager._core_props_path = core_props_path

    def _validate_content_types(self) -> bool:
        """Validate [Content_Types].xml structure."""
        try:
            with self.zip_accessor.get_zip_file() as zf:
                content_types = self.metadata_manager._read_content_types(zf)

                # Check for .rels default
                has_rels_default = any(
                    d.extension == "rels" and d.content_type == self.RELS_CONTENT_TYPE
                    for d in (content_types.default or [])
                )

                if not has_rels_default:
                    logging.warning("[EPC Structure] Missing or incorrect .rels default content type")
                    return False

                # Check for Core Properties override
                core_props_path = gen_core_props_path(self.export_version)
                has_core_props = any(
                    o.part_name and o.part_name.lstrip("/") == core_props_path for o in (content_types.override or [])
                )

                if not has_core_props:
                    logging.warning("[EPC Structure] Core Properties not declared in Content Types")
                    return False

                return True

        except Exception as e:
            logging.error(f"[EPC Structure] Error validating Content Types: {e}")
            return False

    def _validate_root_relationships(self) -> bool:
        """Validate _rels/.rels structure."""
        try:
            with self.zip_accessor.get_zip_file() as zf:
                try:
                    rels_xml = zf.read("_rels/.rels")
                    root_rels = read_energyml_xml_bytes(rels_xml, Relationships)

                    # Check for Core Properties relationship
                    has_core_props_rel = any(
                        r.type_value == self.CORE_PROPS_REL_TYPE for r in (root_rels.relationship or [])
                    )

                    if not has_core_props_rel:
                        logging.warning("[EPC Structure] Core Properties relationship missing from root rels")
                        return False

                    return True

                except KeyError:
                    logging.warning("[EPC Structure] _rels/.rels file missing")
                    return False

        except Exception as e:
            logging.error(f"[EPC Structure] Error validating root relationships: {e}")
            return False

    def _validate_core_properties(self) -> bool:
        """Validate Core Properties file existence."""
        try:
            core_props_path = gen_core_props_path(self.export_version)
            with self.zip_accessor.get_zip_file() as zf:
                try:
                    zf.getinfo(core_props_path)
                    return True
                except KeyError:
                    logging.warning(f"[EPC Structure] Core Properties file missing: {core_props_path}")
                    return False

        except Exception as e:
            logging.error(f"[EPC Structure] Error validating Core Properties: {e}")
            return False

    def _repair_content_types(self) -> None:
        """Repair [Content_Types].xml."""
        try:
            with self.zip_accessor.get_zip_file() as source_zf:
                try:
                    content_types = self.metadata_manager._read_content_types(source_zf)
                except:
                    # Create new if doesn't exist
                    content_types = Types()

                # Ensure .rels default
                if not content_types.default:
                    content_types.default = []

                has_rels = any(d.extension == "rels" for d in content_types.default)
                if not has_rels:
                    content_types.default.append(Default(extension="rels", content_type=self.RELS_CONTENT_TYPE))
                    logging.info("[EPC Structure] Added .rels default content type")

                # Ensure Core Properties override
                if not content_types.override:
                    content_types.override = []

                core_props_path = gen_core_props_path(self.export_version)
                has_core_props = any(
                    o.part_name and o.part_name.lstrip("/") == core_props_path for o in content_types.override
                )

                if not has_core_props:
                    content_types.override.append(
                        Override(part_name=f"/{core_props_path}", content_type=self.CORE_PROPS_CONTENT_TYPE)
                    )
                    logging.info("[EPC Structure] Added Core Properties to Content Types")

                # Write back
                self._write_to_zip(get_epc_content_type_path(), serialize_xml(content_types))

        except Exception as e:
            logging.error(f"[EPC Structure] Error repairing Content Types: {e}")
            raise

    def _repair_root_relationships(self) -> None:
        """Repair _rels/.rels."""
        try:
            core_props_path = gen_core_props_path(self.export_version)
            ...
            with self.zip_accessor.get_zip_file() as source_zf:
                try:
                    rels_xml = source_zf.read("_rels/.rels")
                    root_rels = read_energyml_xml_bytes(rels_xml, Relationships)
                except:
                    root_rels = Relationships(relationship=[])

                # Ensure Core Properties relationship exists
                if not root_rels.relationship:
                    root_rels.relationship = []

                has_core_props = any(r.type_value == self.CORE_PROPS_REL_TYPE for r in root_rels.relationship)

                if not has_core_props:
                    root_rels.relationship.append(
                        Relationship(id="CoreProperties", type_value=self.CORE_PROPS_REL_TYPE, target=core_props_path)
                    )
                    logging.info("[EPC Structure] Added Core Properties relationship to root rels")

                # Write back
                self._write_to_zip("_rels/.rels", serialize_xml(root_rels))

        except Exception as e:
            logging.error(f"[EPC Structure] Error repairing root relationships: {e}")
            raise

    def _repair_core_properties(self) -> None:
        """Repair Core Properties file."""
        try:
            core_props_path = gen_core_props_path(self.export_version)
            ...
            core_props = self._create_default_core_properties()
            self._write_to_zip(core_props_path, serialize_xml(core_props))
            logging.info(f"[EPC Structure] Created missing Core Properties at {core_props_path}")
            ...
            logging.error(f"[EPC Structure] Error repairing Core Properties: {e}")
            raise

    def _create_default_core_properties(self) -> CoreProperties:
        """Create default Core Properties object."""
        return CoreProperties(
            created=Created(any_element=epoch_to_date(epoch())),
            creator=Creator(any_element="energyml-utils EpcStreamReader (Geosiris)"),
            identifier=Identifier(any_element=f"urn:uuid:{gen_uuid()}"),
            version="1.0",
        )

    def _write_to_zip(self, path: str, content: str) -> None:
        """Write content to ZIP file using atomic operation."""
        import tempfile
        ...
        try:
            ...
            with self.zip_accessor.get_zip_file() as source_zf:
                with zipfile.ZipFile(temp_path, "w", zipfile.ZIP_DEFLATED) as target_zf:
                    ...
                    # Write new content
                    target_zf.writestr(path, content)

            # Replace original
            import shutil

            shutil.move(temp_path, str(self.zip_accessor.epc_file_path))
            temp_path = None

            # Reopen persistent connection if needed
...
class _RelationshipManager:
    ...
    def update_rels_for_new_object(self, obj: Any, obj_identifier: str) -> None:
        ...
        for dor in direct_dors:
            try:
                ...
                # Create SOURCE relationship : current is referenced by
                ...
                # Create DESTINATION relationship current depends on target
    ...
    def update_rels_for_modified_object(self, obj: Any, obj_identifier: str, old_dors: List[Any]) -> None:
        ...
        for dor in new_dors:
            ...
            # SOURCE relationship : current is referenced by
            source_rel = Relationship(
                target=metadata.file_path,
                type_value=str(EPCRelsRelationshipType.SOURCE_OBJECT),
                id=f"_{gen_uuid()}",
            ...
            # DESTINATION relationship (for added DORs only) current depends on target
    ...
    def write_rels_updates(
        ...
        destination_relationships: List[Relationship],
        source_updates: Dict[str, Relationship],
        ...
        with self.zip_accessor.get_zip_file() as zf:
            ...
            if not delete_source_rels:
                dest_rels_path = self.metadata_manager.gen_rels_path_from_identifier(source_identifier)
                if dest_rels_path:
                    # Read existing rels (excluding DESTINATION_OBJECT type)
                    existing_rels = []
                    try:
                        if dest_rels_path in zf.namelist():
                            rels_data = zf.read(dest_rels_path)
                            existing_rels_obj = read_energyml_xml_bytes(rels_data, Relationships)
                            if existing_rels_obj and existing_rels_obj.relationship:
                                # Keep only non-DESTINATION relationships that will be re-generated from the object
                                existing_rels = [
                                    r
                    ...
                    # Combine with new DESTINATION relationships
                    all_rels = existing_rels + destination_relationships
                    if all_rels:
                        rels_updates[dest_rels_path] = serialize_xml(Relationships(relationship=all_rels))
                    elif dest_rels_path in zf.namelist() and not all_rels:
                        files_to_delete.append(dest_rels_path)
            else:
                # Mark dest rels file for deletion
                dest_rels_path = self.metadata_manager.gen_rels_path_from_identifier(source_identifier)
                if dest_rels_path:
                    files_to_delete.append(dest_rels_path)

            # 2. Handle SOURCE updates the objects that refers to the current
            for target_identifier, source_rel in source_updates.items():
                ...
                # Add new SOURCE relationship if not already present
                rel_exists = any(
                    r.target == source_rel.target and r.type_value == source_rel.type_value for r in existing_rels
                )

                if not rel_exists:
                    existing_rels.append(source_rel)
...
class EpcStreamReader(EnergymlStorageInterface):
    ...
    def __init__(
        ...
        auto_repair_structure: bool = True,
    ):
        """
        ...
        Args:
            ...
            auto_repair_structure: If True, automatically validates and repairs EPC structure on load (default: True)
        """
        # Public attributes
        self.epc_file_path = Path(epc_file_path)
        self.auto_repair_structure = auto_repair_structure
        ...
        self._metadata_mgr.set_export_version(self.export_version)
        self._rels_mgr = _RelationshipManager(
            self._zip_accessor, self._metadata_mgr, self.stats, self.export_version, rels_update_mode
        )
        self._structure_validator = _StructureValidator(self._zip_accessor, self._metadata_mgr, self.export_version)

        # Validate and repair structure (idempotent)
        if auto_repair_structure:
            validation_results = self._structure_validator.validate_and_repair(is_new_file=is_new_file)
            if validation_results["repaired"]:
                logging.info("[EPC Stream] EPC structure has been validated and repaired")
        elif is_new_file:
            # Even without auto-repair, we need to create minimal structure for new files
            self._structure_validator.validate_and_repair(is_new_file=True)
    ...
    @property
    def _uuid_index(self) -> Dict[str, List[str]]:
        """Backward compatibility property for accessing UUID index."""
        return self._metadata_mgr._uuid_index

    @property
    def _type_index(self) -> Dict[str, List[str]]:
        """Backward compatibility property for accessing type index."""
        return self._metadata_mgr._type_index

    @property
    def _core_props(self) -> Optional[CoreProperties]:
        """Backward compatibility property for accessing Core Properties."""
        return self._metadata_mgr._core_props

    @property
    def additional_rels(self) -> Dict[str, List[Relationship]]:
        """Backward compatibility property for accessing additional relationships."""
        return self._rels_mgr.additional_rels

    def _create_empty_epc(self) -> None:
        """
        Create an empty EPC file structure.

        Note: This method is deprecated in favor of _StructureValidator.validate_and_repair().
        It's kept for backward compatibility but now delegates to the structure validator.
        """
        # Ensure directory exists
        self.epc_file_path.parent.mkdir(parents=True, exist_ok=True)

        # Create empty ZIP to allow structure validator to work
...
__all__ = ["EpcStreamReader", "EpcObjectMetadata", "EpcStreamingStats", "read_epc_stream"]
