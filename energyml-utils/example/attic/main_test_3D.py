...
def export_all_representation(epc_path: str, output_dir: str, regex_type_filter: Optional[str] = None):
...
def export_all_representation_in_memory(epc_path: str, output_dir: str, regex_type_filter: Optional[str] = None):

    storage = Epc.read_file(epc_path)
    if storage is None:
        logging.error(f"Failed to read EPC file: {epc_path}")
        return
